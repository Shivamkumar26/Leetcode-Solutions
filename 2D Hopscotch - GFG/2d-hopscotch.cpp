...
public:
    int hopscotch(int n, int m, vector<vector<int>> mat, int ty, int i, int j) {
        ...
        int ans = 0;
        if(ty == 0) {
            if(j%2 == 0) ans = fun(n,m,mat,dx1e,dy1e,i,j,6);
            else ans = fun(n,m,mat,dx1o,dy1o,i,j,6);
        }
        else {
            if(j%2 == 0) ans = fun(n,m,mat,dx2e,dy2e,i,j,12);
            else ans = fun(n,m,mat,dx2o,dy2o,i,j,12);
        }
        return ans;
    }
};
